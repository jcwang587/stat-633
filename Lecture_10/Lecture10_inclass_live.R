...
# 1. Family tree: nodes = family members, edges = descendency, sibling, spouse, ownership of a pet
# 2. Social media network: Twitter: nodes = accounts, edges = followers. 
...
# Other major topics in networks science involve, for example: 
...
# Answer:
# Since we can have nodes that are not connected to any edge, we must 
# retain the list of nodes.

# Question: How can we make this into a directed network?
# Answer: 
# We could simply label one column in the ist of edges as "from" and the other "to".
...
# What should we do if we wanted a similar visualization for the a simple network?
heatmaply(
  matrix_UKfaculty,
  Rowv = row_dend,
  Colv = row_dend
...
as.matrix(as_adjacency_matrix(net, attr = "weight"))
