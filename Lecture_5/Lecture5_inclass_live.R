...
plot_ly(diamonds, x = ~cut)   # aes(x = cut)
...
plot_ly(
  ...
  x = ~cut,
  color = I("red"),
  stroke = I(rgb(218/256, 7/256, 59/256)),
  span = I(5)
)
...
# Want: x = label of cut, y = count of each category of cut
...
plotly_dat1 <- 
  ...
  plotly_data()
plotly_dat1
all.equal(diamonds, plotly_dat1)
...
p1 %>% layout(showlegend = FALSE)


# Example:
# How do we change the background color of the modebar to red? 
# (notice the hierarchical structure):
p1 %>% layout(modebar = list(bgcolor = "red", orientation = "v"))


# Exercise:
# i. Get rid of `trace0` text in the tooltip for the histogram. 
# Get rid of the tooltip for the text layer entirely.
# (hint: `hoverinfo`).
...
diamonds %>%
  ...
  layout(hoverlabel = list(bgcolor = "yellow", font = list(color = "red")))


diamonds %>%
  plot_ly( x = ~cut ) %>%
  add_histogram(
    hoverlabel = list(bgcolor = "red", font = list(color = "yellow")),
    hoverinfo = "x+y"
    ) %>%
  group_by( cut ) %>%
  summarise(n = n()) %>%
  add_text(
    ...
    textposition = "top middle",
    cliponaxis = FALSE,
    hoverlabel = list(bgcolor = "yellow", font = list(color = "red")),
    hoverinfo = "x+y"
  ) %>%
...
# ii. change the color of the font in the tooltip to red and the background of the 
#     label to yellow (hint: `hoverlabel`).
